...
class GetAllUserDetailsApi {
  ...
  static Future<GetAllUserDetailsApi> call() async {
    Responce res = Responce();
    UtilsVariables.token = await SharedPre.getToken();
    String userid = await SharedPre.getUserid();
    res = await ServiceGet.callApi(
        '${UtilsVariables.url}/GetAllUsers?userId=$userid',
        UtilsVariables.token);

    return GetAllUserDetailsApi.fromJson(res);
  }

  factory GetAllUserDetailsApi.fromJson(Responce res) {
    if (res.resCode! <= 210 && res.resCode! >= 200) {
      var data2 = jsonDecode(res.responceBody!.toString()) as List;
      if (data2.isNotEmpty) {
        List<AllUserData> datalist =
            data2.map((e) => AllUserData.fromJson(e)).toList();
        return GetAllUserDetailsApi(
